...
export function buttonStyles(
  ...
  return cn(
    ...
    variant === 'primary' && 'border border-[#267e63] bg-[#267e63] text-white hover:bg-[#1f6c55]',
    variant === 'secondary' && 'border border-slate-300 bg-white text-slate-800 hover:border-slate-400 hover:bg-slate-50',
    variant === 'ghost' && 'border border-transparent bg-transparent text-slate-600 hover:bg-slate-100 hover:text-slate-900',
...
export function Panel({ className, ...props }: HTMLAttributes<HTMLDivElement>) {
  return <div className={cn('app-panel rounded-lg border p-4', className)} {...props} />
}
...
export function EmptyState({ children, className }: PropsWithChildren<{ className?: string }>) {
  return (
    <div className={cn('rounded-lg border border-dashed border-slate-300 bg-slate-50/90 p-4 text-sm text-slate-500', className)}>
      {children}
    </div>
  )
}
...
export const fieldStyles =
  'app-field w-full rounded-md border border-slate-200 bg-white px-3 py-2 text-sm text-slate-950 outline-none transition placeholder:text-slate-400 hover:border-slate-300 focus:border-emerald-700 focus:ring-2 focus:ring-emerald-700/10'
