    ...
    <PageTransition className="grid h-full min-h-0 grid-rows-[auto_minmax(0,1fr)] gap-4">
      ...
      {rows.length ? (
        <Panel className="grid min-h-0 grid-rows-[auto_minmax(0,1fr)_auto] overflow-hidden p-0">
          ...
          <div className="app-section-band grid gap-2 border-b border-slate-200 p-3 md:grid-cols-2">
            {orderFiles.map((file) => (
              <div key={file.order.id} className="app-soft-card rounded-md border px-3 py-2">
                ...
                <div className="grid gap-1.5">
                  {orderFiles.map((file) => {
                    ...
                    return (
                      <div
                        key={file.order.id}
                        className="app-soft-card flex flex-wrap items-center justify-between gap-2 rounded-md border px-2.5 py-2"
                      >
                        <div className="min-w-0">
                        ...
                        <Button
                          ...
                          className={cn(
                            ...
                            active && 'min-w-[252px] border-slate-300 bg-slate-100 text-slate-950',
                          )}
      ...
      {formationStatus === 'done' ? (
        <div className="app-soft-card fixed bottom-5 left-1/2 z-50 -translate-x-1/2 rounded-lg border px-4 py-3 text-sm text-emerald-900">
